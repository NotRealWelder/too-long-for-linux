...
def parse_arguments():
    ...
    parser.add_argument(
        "--axe",
        action="store_true",
        help="Автоматически исправить длинные имена файлов и директорий (использовать с осторожностью!!! длинные пути не исправляются, могут потребоваться права root)",
    )
